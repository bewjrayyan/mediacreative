...
@section('content')
<section class="hero">
    <div class="hero-text"><span class="eyebrow">Content · Pages</span><h1 class="hero-title">Add <span class="accent">page</span></h1></div>
    <div class="hero-actions">
        <a href="{{ route('admin.pages.index') }}" class="btn btn--ghost">Cancel</a>
        <button form="form" type="submit" class="btn btn--primary">Create Page</button>
    </div>
</section>
<section class="card">
    <div class="card-head"><div class="card-title-wrap"><span class="eyebrow">Form</span><h2 class="card-title">Page details</h2></div></div>
    <form id="form" method="POST" action="{{ route('admin.pages.store') }}">
        @csrf
        <div class="grid">
            <div class="col-6">
            ...
            <div class="col-12">
                <div class="field"><label class="field-label">Meta Description</label>
                    <textarea class="input" name="meta_description" rows="2">{{ old('meta_description') }}</textarea>
                </div>
            </div>
            <div class="col-12">
                <div class="field"><label class="field-label">Content (HTML) *</label>
                    <textarea class="input monospace" name="content" rows="14" style="font-family:'JetBrains Mono',monospace;font-size:13px" required>{{ old('content') }}</textarea>
                </div>
            </div>
        </div>
    </form>
